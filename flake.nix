...
#
# Development environment for network-dashboard.
#
...
# (just elixir) — not a generic estate default.
#
#   nix develop      # enter the shell
#   nix flake check  # verify this file evaluates (run before committing)
{
  description = "network-dashboard development environment";
    ...
    in
    {
      devShells = forAllSystems (pkgs: {
        default = pkgs.mkShell {
          packages = with pkgs; [ just elixir ];
        };
      });
    };
}
